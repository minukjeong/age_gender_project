...
class FaceDataset(Dataset):
  ...
  def __init__(self, datapath, transformer):
    ...
    #mefix
    #self.age_cls = {x-1: self.GaussianProb(x) for x in range(1, self.age_cls_unit + 1)}
    #self.age_cls_zeroone = {x-1: self.ZeroOneProb(x) for x in range(1, self.age_cls_unit + 1)}
    self.age_cls = {x : self.GaussianProb(x) for x in range(2, self.age_cls_unit + 1)}
    self.age_cls_zeroone = {x : self.ZeroOneProb(x) for x in range(2, self.age_cls_unit + 1)}

  def __len__(self):
    return len(self.pics)

  def GaussianProb(self, true, var = 2.5):
    #x = np.array(range(0, self.age_cls_unit), dtype='float')
  ...
  def __getitem__(self, idx):
    ...
    age = max(0., min(float(age), float(self.age_cls_unit)))
    ...
    age_rgs_label = age_rgs_label.type(torch.LongTensor)
